...
impl MarkdownPolicy for PlainTextPolicy {
    fn markdown_format_instructions(&self) -> String {
        "Your responses should be plain text. Do not use any markdown formatting.".to_string()
    }
                    ...
                    for part in content_parts {
                        match part {
                            ...
                            AssistantContent::Thought(thought) => {
                                assistant_content_str.push_str(&format!("[THOUGHT]: {}", thought));
                            },
                            AssistantContent::ToolCall(tool_call) => {
                                assistant_content_str.push_str(&format!("[TOOL_CALL]: {}({})", tool_call.name, serde_json::to_string(&tool_call.arguments)?));
                            },
                        }
                    }
                    llm_chat_messages.push(LlmChatMessage {
