...
pub enum GitError {
    ...
    FindTreeError(#[source] git2::Error),
    #[error("Failed to create signature")]
    SignatureError(#[source] git2::Error),
    #[error("Failed to create commit")]
    CreateCommitError(#[source] git2::Error),
    #[error("Failed to find the new commit")]
    FindNewCommitError(#[source] git2::Error),
    #[error("Failed to get tree from the new commit")]
    NewCommitTreeError(#[source] git2::Error),
    #[error("Failed to restore index to new HEAD state")]
    RestoreIndexError(#[source] git2::Error),
    #[error("Failed to write restored index")]
    WriteRestoredIndexError(#[source] git2::Error),
}
 
pub fn git_commit_files(files_to_commit: &[PathBuf], message: &str) -> Result<(), GitError> {
    ...
    // and the base for "cleaning" the staging area.
        ...
        let head_tree = head_commit.tree().map_err(GitError::Git2)?;
    
        // 3. Load the repository index.
        let mut index = repo.index().map_err(GitError::Git2)?;
        ...
        let statuses = repo.statuses(Some(&mut status_options)).map_err(GitError::Git2)?;
        ...
        index.read_tree(&head_tree).map_err(GitError::Git2)?;
    
        // 6. Selectively add files_to_commit to the staging area (index).
        for path in files_to_commit {
            let canonical_path = path.canonicalize().map_err(|e| GitError::CanonicalizePathError(path.clone(), e))?;
            let canonical_workdir = workdir.canonicalize().map_err(|e| GitError::CanonicalizePathError(workdir.to_path_buf(), e))?;
    
            let relative_path = canonical_path.strip_prefix(&canonical_workdir).map_err(|_| {
                GitError::FileOutsideWorkdir(path.clone(), workdir.to_path_buf())
            })?;
            index.add_path(relative_path).map_err(|e| GitError::AddPathError(path.clone(), e))?;
        }
    
        // 7. Write the changes to the index to disk.
        index.write().map_err(GitError::WriteIndexError)?;
    
        // 8. Create a new tree based on the current state of the index. 
        let tree_oid = index.write_tree().map_err(GitError::WriteTreeError)?;
        let tree = repo.find_tree(tree_oid).map_err(GitError::FindTreeError)?;
    
        // 9. Create the author and committer signature.
        let signature = Signature::now("vespe", "vespe@example.com").map_err(GitError::SignatureError)?;
        ...
        .map_err(GitError::CreateCommitError)?;
    
        // 11. Reset the index to the *new* HEAD. This is crucial to avoid the "ghost" effect
        //     for the newly committed files.
        let new_head_commit = repo.find_commit(new_commit_oid).map_err(GitError::FindNewCommitError)?;
        let new_head_tree = new_head_commit.tree().map_err(GitError::NewCommitTreeError)?;
    
        index.read_tree(&new_head_tree).map_err(GitError::RestoreIndexError)?;
        ...
        index.write().map_err(GitError::WriteRestoredIndexError)?;
