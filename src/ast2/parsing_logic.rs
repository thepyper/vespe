...
use crate::ast2::types::{Anchor, AnchorKind, Argument, Arguments, CommandKind, Content, Parameters, Range, Text};
...
pub fn _try_parse_tag<'doc>(parser: &Parser<'doc>) -> Result<Option<(crate::ast2::types::Tag, Parser<'doc>)>> {
...
fn _try_parse_enclosed_string<'doc>(
    ...
    loop {
        if let Some(p) = current_parser.consume_matching_string_immutable("\"") {
            ...
            value.push('\'');
            current_parser = p;
        } else if let Some(_p) = current_parser.consume_matching_string_immutable("\\\\") {
            value.push('\\');
