...
impl FileBasedEditorCommunicator {
    pub fn new(path: &Path) -> Result<Self> {
        ...
        if let Some(parent) = request_file.parent() {
            fs::create_dir_all(parent).map_err(EditorError::CreateDirFailed)?;
        }
        ...
        fs::write(&request_file, serde_json::to_string(&RequestState::None).map_err(EditorError::SerializeJsonFailed)?)?;
        fs::write(&response_file, serde_json::to_string(&ResponseState::None).map_err(EditorError::SerializeJsonFailed)?)?;
        ...
        env::set_var(
            ...
            request_file
                .to_str()
                .ok_or_else(|| EditorError::InvalidFilePath("Invalid request file path".to_string()))?,
        );
        env::set_var(
            "VESPE_RESPONSE_FILE_PATH",
            response_file
                .to_str()
                .ok_or_else(|| EditorError::InvalidFilePath("Invalid response file path".to_string()))?,
        );
    ...
    fn _write_request(&self, state: RequestState) -> Result<()> {
        let json = serde_json::to_string_pretty(&state).map_err(EditorError::SerializeJsonFailed)?;
        fs::write(&self.request_file_path, json).map_err(EditorError::WriteFileFailed)?;
        Ok(())
    }
    ...
    fn _read_response(&self, expected_request_id: Uuid) -> Result<ResponseState> {
        ...
        let mut attempts = 0;
        loop {
            let content = fs::read_to_string(&self.response_file_path).map_err(EditorError::ReadFileFailed)?;
            let response: ResponseState = serde_json::from_str(&content).map_err(EditorError::DeserializeJsonFailed)?;
                ...
                | ResponseState::Error { request_id, .. } => {
                    if *request_id == expected_request_id {
                        ...
                        fs::write(
                            ...
                            serde_json::to_string(&ResponseState::None).map_err(EditorError::SerializeJsonFailed)?,
                        ).map_err(EditorError::WriteFileFailed)?;
                        return Ok(response);
                    }
                }
            ...
            if attempts > 60 {
                ...
                return Err(EditorError::Timeout.into());
            }
    ...
    fn request_file_modification(&self, file_path: &Path) -> Result<Uuid> {
        ...
        match response {
            ...
            ResponseState::Error { message, .. } => {
                Err(EditorError::EditorResponseError(message).into())
            }
            _ => Err(EditorError::UnexpectedEditorResponse.into()),
        }
    }
