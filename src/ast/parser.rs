...
pub fn parse_line(input: &str) -> Result<Line, PestError<Rule>> {
    let mut pairs = AstParser::parse(Rule::line, input)?;
    let pair = pairs.next().unwrap(); // Get the top-level 'line' rule

    let mut line_kind = LineKind::Text(String::new()); // Default to Text
