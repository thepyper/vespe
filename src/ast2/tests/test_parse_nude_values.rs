...
fn test_try_parse_nude_integer() {
    ...
    let (value, p_next) = super::super::_try_parse_nude_integer(&parser).unwrap().unwrap();
    assert_eq!(value, 123);
    assert_eq!(p_next.remain(), " rest");

    let doc_no_int = "abc";
    let parser_no_int = Parser::new(doc_no_int);
    assert!(super::super::_try_parse_nude_integer(&parser_no_int).unwrap().is_none());

    let doc_empty = "";
    let parser_empty = Parser::new(doc_empty);
    assert!(super::super::_try_parse_nude_integer(&parser_empty).unwrap().is_none());
}
...
fn test_try_parse_nude_float() {
    let doc = "123.45 rest";
    let parser = Parser::new(doc);
    let (value, p_next) = super::super::_try_parse_nude_float(&parser).unwrap().unwrap();
    assert_eq!(value, 123.45);
    assert_eq!(p_next.remain(), " rest");

    let doc_no_float = "123 rest";
    let parser_no_float = Parser::new(doc_no_float);
    assert!(super::super::_try_parse_nude_float(&parser_no_float).unwrap().is_none());

    let doc_just_dot = ". rest";
    let parser_just_dot = Parser::new(doc_just_dot);
    assert!(super::super::_try_parse_nude_float(&parser_just_dot).unwrap().is_none());

    let doc_empty = "";
    let parser_empty = Parser::new(doc_empty);
    assert!(super::super::_try_parse_nude_float(&parser_empty).unwrap().is_none());
}
...
fn test_try_parse_nude_bool() {
    ...
    let (value_true, p_next_true) = super::super::_try_parse_nude_bool(&parser_true).unwrap().unwrap();
    assert_eq!(value_true, true);
    assert_eq!(p_next_true.remain(), " rest");

    let doc_false = "false rest";
    let parser_false = Parser::new(doc_false);
    let (value_false, p_next_false) = super::super::_try_parse_nude_bool(&parser_false).unwrap().unwrap();
    ...
    assert!(super::super::_try_parse_nude_bool(&parser_no_bool).unwrap().is_none());
}
...
fn test_try_parse_nude_string() {
    ...
    let parser = Parser::new(doc);
    let (value, p_next) = super::super::_try_parse_nude_string(&parser).unwrap().unwrap();
    assert_eq!(value, "hello/world.txt_123");
    assert_eq!(p_next.remain(), " rest");

    let doc_empty = "";
    let parser_empty = Parser::new(doc_empty);
    assert!(super::super::_try_parse_nude_string(&parser_empty).unwrap().is_none());

    let doc_with_space = "hello world";
    let parser_with_space = Parser::new(doc_with_space);
    let (value_space, p_next_space) = super::super::_try_parse_nude_string(&parser_with_space).unwrap().unwrap();
...
fn test_try_parse_value_nude() {
    ...
    let (value_int, p_next_int) = super::super::_try_parse_value(&parser_int).unwrap().unwrap();
    ...
    let (value_float, p_next_float) = super::super::_try_parse_value(&parser_float).unwrap().unwrap();
    ...
    let (value_bool, p_next_bool) = super::super::_try_parse_value(&parser_bool).unwrap().unwrap();
    ...
    let (value_string, p_next_string) = super::super::_try_parse_value(&parser_string).unwrap().unwrap();
    assert_eq!(value_string, json!("nude_string"));
    assert_eq!(p_next_string.remain(), " rest");

    let doc_empty = "";
    let parser_empty = Parser::new(doc_empty);
    assert!(super::super::_try_parse_value(&parser_empty).unwrap().is_none());
}
