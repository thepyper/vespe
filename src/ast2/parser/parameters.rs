...
pub(crate) fn _try_parse_parameters<'doc>(
    ...
    if let Some((json_object, parser)) = super::json_plus::_try_parse_jsonplus_object(parser)? {
...
pub(crate) fn _try_parse_parameter<'doc>(
    ...
    let (key, p1) = match super::values::_try_parse_identifier(&p_initial)? {
        Some((k, p)) => (k, p),
    ...
    let (value, p5) = match super::values::_try_parse_value(&p4) {
        Ok(Some((v, p))) => (v, p),
        Ok(None) => {
