...
Ora, vorrei implementassi anche un AsyncVisitor in cui tutte le visite sono asincrone.
Puoi farlo nello stesso src/ast/visitor.rs dell'altro.
Chiaro?

Okay, l'implementazione dell'AsyncVisitor è stata completata e commessa.

Ora, devo risolvere i problemi rimanenti nei test:
1.  **Errori "file not found"**: Devo eseguire il debug di `src/ast/parser.rs` per capire come `parse_file_to_lines` e le funzioni di parsing gestiscono i percorsi risolti da `MockResolver`.
2.  **Errore di asserzione in `test_parse_line_with_answer_tag_and_anchor`**: Questo indica un problema con la regex o la manipolazione delle stringhe durante l'analisi del tag `@answer`.

Inizierò rileggendo `src/ast/parser.rs` per eseguire il debug della logica di lettura dei file e dell'analisi dei tag.

***********

Togli i suffissi _async dalle funzioni async, sono ineleganti.
Aggiungi al visito non async il return type Result<()>, gestisci errori in walk uscendo al primo errore trovato.
