use anyhow::Result;
use crate::ast2::{parse_document, Document, Range, Tag, Anchor, CommandKind, AnchorKind};
...
impl ContentItem {
    ...
    fn agent(author: &str, text: &str) -> Self {
        ContentItem::Agent(AgentContent { author: author.into(), text: text.into() })
    }
}
...
impl Tag {
    ...
    pub fn validate_argument_as_context(&self, i: usize, project: &Project) -> Result<PathBuf> {
        ...
        match std::fs::exists(context_path) {
            ...
            false => // TODO inesistente
        }
    }
}

type serde_json::Value = State;
...
impl Executor {
    ...
    fn pass_1_anchor(&self, anchor: &Anchor) -> Result<bool> {
        ...
        match (anchor.command, anchor.kind, anchor.parameters, anchor.arguments) {
            (CommandKind::Answer, AnchorKind::Begin, parameters, arguments) => want_next_step |= self.pass_1_answer_begin_anchor(state, parameters, arguments), // passa commit?
            (CommandKind::Derive, AnchorKind::Begin, parameters, arguments) => want_next_step |= self.pass_1_derive_begin_anchor(state, parameters, arguments), // passa commit?
            (CommandKind::Inline, AnchorKind::Begin, parameters, arguments) => want_next_step |= self.pass_1_inline_begin_anchor(state, parameters, arguments), // passa commit?
            (CommandKind::Repeat, AnchorKind::Begin, parameters, arguments) => want_next_step |= self.pass_1_repeat_begin_anchor(state, parameters, arguments), // passa commit?
            (CommandKind::Summarize, AnchorKind::Begin, parameters, arguments) => want_next_step |= self.pass_1_summarize_begin_anchor(state, parameters, arguments), // passa commit?
            _ => {}
        }
    }

    fn pass_1_answer_begin_anchor(&self, state: &State, parameters: &Parameters, arguments: &Arguments) -> Result<bool> {
        unimplemented!()
    }

    fn pass_1_derive_begin_anchor(&self, state: &State, parameters: &Parameters, arguments: &Arguments) -> Result<bool> {
        unimplemented!()
    }

    fn pass_1_inline_begin_anchor(&self, state: &State, parameters: &Parameters, arguments: &Arguments) -> Result<bool> {
        unimplemented!()
    }

    fn pass_1_repeat_begin_anchor(&self, state: &State, parameters: &Parameters, arguments: &Arguments) -> Result<bool> {
        unimplemented!()
    }

    fn pass_1_summarize_begin_anchor(&self, state: &State, parameters: &Parameters, arguments: &Arguments) -> Result<bool> {
        unimplemented!()
    }
