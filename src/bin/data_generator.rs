...
async fn label_student_response<'a>(
    client: &Client,
    args: &CliArgs,
    student_prompt: &'a str,
    student_response: &'a str,
    system_prompt_used: &'a str,
) -> anyhow::Result<String> {
...
========== Inizio Esempio {}/{} ==========", i, args.num_examples);
        ...
        if let Err(e) = save_labeled_example(&args.output_dir, &labeled_json, i).await {
